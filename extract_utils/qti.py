...
class ExtractUtilsQTIModule(ExtractUtilsModule):
    ...
    def __init__(
        self,
        component: str,  # Component name (e.g. 'adreno-t', 'display/5.15')
        component_type: QTIComponentType,  # SYSTEM or VENDOR
        vendor: str = 'qcom',
    ...
    def get_component_path(self) -> str:
        """Get the full path for this QTI component."""
        return path.join(self.vendor_path, self.component_type, self.component)

    def get_blobs_path(self) -> str:
        """Get the path where blobs should be extracted for this component."""
        return path.join(self.get_component_path(), 'proprietary')

    def should_skip_file(self, file_path: str) -> bool:
        """
        Determine if a file should be skipped during extraction.
        This can be overridden by subclasses to implement custom skip logic.
        """
        return False
